...
let op2 = [
    ...
    opcode('je', (s, a, b, c, d) => {
        ...
        log.debug(`${hex(s.op_pc)} je ${hex(a)} ${hex(b)} ${c !== undefined ? hex(c) : ''} ${d !== undefined ? hex(d) : ''} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
        let cond = (a === b ||
                    (c !== undefined && a === c) ||
                    (d !== undefined && a === d));
    ...
    opcode('jl', (s, a, b) => {
        let [offset,condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} jl ${hex(a)} ${hex(b)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
        s.doBranch(s.toI16(a) < s.toI16(b), condfalse, offset);
    }),
    opcode('jg', (s, a, b) => {
        let [offset, condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} jg ${hex(a)} ${hex(b)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
        s.doBranch(s.toI16(a) > s.toI16(b), condfalse, offset);
    }),
    opcode('dec_chk', (s, variable, value) => {
        let [offset, condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} dec_chk ${hex(variable)} ${value} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
    ...
    opcode('inc_chk', (s, variable, value) => {
        let [offset, condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} inc_chk ${hex(variable)} ${value} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
        let new_val = s.toI16(s.loadVariable(variable)) + 1;
        s.storeVariable(variable, s.toU16(new_val));
        log.debug(`     ${new_val} ?> ${value}`);
        s.doBranch(new_val > s.toI16(value), condfalse, offset);
    ...
    opcode('jin', (s, obj1, obj2) => {
        let [offset, condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} jin ${hex(obj1)} ${hex(obj2)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
    ...
    opcode('test', (s, bitmap, flags) => {
        let [offset, condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} test ${hex(bitmap)} ${hex(flags)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
...
//fs.writeSync(log_fp, `+  ${hex(obj)} / ${hex(attribute)}\n`);
        let [offset, condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} test_attr ${hex(obj)} ${hex(attribute)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
    ...
    opcode('loadw', (s, array, word_index) => {
        let resultVar = s.readByte();
        log.debug(`${hex(s.op_pc)} loadw ${hex(array)} ${hex(word_index)} -> (${hex(resultVar)})`);
        s.storeVariable(resultVar, s.getWord((array + 2 * word_index) & 0xffff));
    }),
    opcode('loadb', (s, array, byte_index) => {
        let resultVar = s.readByte();
        log.debug(`${hex(s.op_pc)} loadb ${hex(array)} ${hex(byte_index)} -> (${hex(resultVar)})`);
        s.storeVariable(resultVar, s.getByte((array + byte_index) & 0xffff));
    }),
    opcode('get_prop', (s, obj, property) => {
        let resultVar = s.readByte();
        log.debug(`${hex(s.op_pc)} get_prop ${hex(obj)} ${hex(property)} -> (${hex(resultVar)})`);
        let o = s.getObject(obj);
        if (o === null) {
...
//fs.writeSync(log_fp, ` get_prop_addr ${hex(obj)} ${hex(property)}`);
        let resultVar = s.readByte();
        log.debug(`${hex(s.op_pc)} get_prop_addr ${hex(obj)} ${hex(property)} -> (${hex(resultVar)})`);
        let o = s.getObject(obj);
        s.storeVariable(resultVar, o.getPropertyAddress(property));
    }),
    opcode('get_next_prop', (s, obj, property) => {
        let resultVar = s.readByte();
        log.debug(`${hex(s.op_pc)} get_next_prop ${hex(obj)} ${hex(property)} -> (${hex(resultVar)})`);
        let o = s.getObject(obj);
    ...
    opcode('call_2s', (s, routine, arg1) => {
        ...
        log.debug(`${hex(s.op_pc)} call_2s ${hex(routine)} ${arg1} -> (${hex(resultVar)})`);
    ...
    opcode('set_color', (s, foreground, background) => {
        log.warn(`${hex(s.op_pc)} set_color ${foreground} ${background} -- not implemented`);
    }),
...
let op1 = [
    opcode('jz', (s, a) => {
        let [offset, condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} jz ${hex(a)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
        s.doBranch(a === 0, condfalse, offset);
    }),
    opcode('get_sibling', (s, obj) => {
        let resultVar = s.readByte();
        let [offset, condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} get_sibling ${hex(obj)} -> (${hex(resultVar)}) ?[${!condfalse}] ${hex(offset)}`);
...
//fs.writeSync(log_fp, `+  sibling = ${sibling ? hex(sibling.objnum) : '0'}\n`);
    ...
    opcode('get_child', (s, obj) => {
        let resultVar = s.readByte();
        let [offset, condfalse] = s.readBranchOffset();
        log.debug(`${hex(s.op_pc)} get_child ${hex(obj)} -> (${hex(resultVar)}) ?[${!condfalse}] ${hex(offset)}`);
    ...
    opcode('get_parent', (s, obj) => {
        ...
        let o = s.getObject(obj);
        if (o === null)
            log.error("object null in get_parent");
    ...
    opcode('get_prop_len', (s, propDataAddr) => {
      let resultVar = s.readByte();
      log.debug(`${hex(s.op_pc)} get_prop_len ${hex(propDataAddr)} -> (${hex(resultVar)})`);
...
let opv = [
    opcode('call', (s, routine, ...args) => {
        let resultVar = s.readByte();
        routine = s.unpackRoutineAddress(routine);
        log.debug(`${hex(s.op_pc)} call ${hex(routine)} ${args} -> (${hex(resultVar)})`);
        s.callRoutine(routine, resultVar, ...args);
    }),
    opcode('storew', (s, array, word_index, value) => {
      log.debug(`${hex(s.op_pc)} storew ${hex(array)} ${hex(word_index)} ${hex(value)}`);
      s.setWord((array + 2*word_index) & 0xffff, value);
    }),
    opcode('storeb', (s, array, byte_index, value) => {
      log.debug(`${hex(s.op_pc)} storeb ${hex(array)} ${hex(byte_index)} ${hex(value)}`);
    ...
    opcode('call_vs2', (s, routine, ...args) => {
        let resultVar = s.readByte();
        routine = s.unpackRoutineAddress(routine);
        log.debug(`${hex(s.op_pc)} call_vs2 ${hex(routine)} ${args} -> (${hex(resultVar)})`);
        s.callRoutine(routine, resultVar, ...args);
    }),
...
//fs.writeSync(log_fp, ` ${hex(varval)}`);
    ...
    try {
        ...
        log.error(`error at pc=${hex(op_pc)}, opcode=${hex(opcode)}: ${e.toString()}`);
        throw e;
    }
    impl(s, ...operands);
...
class GameObject {
    ...
    dump(indent = 0) {
        ...
        for (;;) {
            ...
            if (propNum === 0) break;
            console.log(`${_indent}   ${hex(entry)} [${propNum}] ${this.dumpPropData(entry)}`);
    ...
    get name() { return zstringToAscii(this.state, this.state.getZString(this.propertyTableAddr + 1), false) }

    get parent() {
        return this.state.getObject((this.state._version <= 3) ? this.state.getByte(this.objaddr + 4) : this.state.getWord(this.objaddr + 6));
    }
    set parent(po) {
    ...
    get child() {
        return this.state.getObject((this.state._version <= 3) ? this.state.getByte(this.objaddr + 6) : this.state.getWord(this.objaddr + 10));
    }
    set child(co) {
    ...
    get sibling() {
        return this.state.getObject((this.state._version <= 3) ? this.state.getByte(this.objaddr + 5) : this.state.getWord(this.objaddr + 8));
    }
    set sibling(so) {
    ...
    get propertyTableAddr() { return this.state.getWord(this.objaddr + (this.state._version <= 3 ? 7 : 12)); }
    ...
    getProperty(prop) {
        ...
        switch (propLen) {
        case 1: return this.state.getByte(this._propDataPtr(propAddr));
        case 2: return this.state.getWord(this._propDataPtr(propAddr));
        default: throw new Error(`invalid property length in getProperty: ${propLen}`);
        }
    }
    ...
    putProperty(prop, value) {
        ...
        let propLen = GameObject._propDataLen(this.state, propAddr);
        switch (propLen) {
        case 1: return this.state.setByte(this._propDataPtr(propAddr), value & 0xFF);
        case 2: return this.state.setWord(this._propDataPtr(propAddr), value & 0xFFFF);
        default: throw new Error(`invalid property length in getProperty: ${propLen}`);
        }
    }
...
class Game {
    ...
    dumpDictionary() {
        ...
        for (let i = 0; i < num_sep; i ++)
            sep_zscii.push(this.getByte(p++));

        console.log(`Separators: ${sep_zscii.map((ch) => String.fromCharCode(ch)).join(" ")}`);

        let entry_length = this.getByte(p++);
        let num_entries = this.getWord(p); p+=2;

        for (let i = 0; i < num_entries; i ++) {
            let entry_text = zstringToAscii(this, this.getZString(p), false);
            console.log(` [${i}] ${entry_text} ${hex(this.getWord(p))} ${hex(this.getWord(p+2))}`);
    ...
    pushStack(v) {
        ...
        this._stack.push(v);
        log.debug(`     after pushStack(${hex(v)}): ${this._stack.map((el) => hex(el))}`);
    }
    popStack() {
    ...
    storeVariable(v, value, replaceTop=false) {
      ...
      if (v < 16) {
        ...
        if (v > cur_frame.locals.length) { throw new Error(`local ${v} out of range.  there are ${cur_frame.locals.length} locals`); }
    ...
    loadVariable(variable, peekTop=false) {
      ...
      if (variable < 16) {
        // local
        let cur_frame = this._callstack[this._callstack.length-1];
        if (variable > cur_frame.locals.length) { throw new Error("no local"); }
        return cur_frame.locals[variable-1];
      }
      else {
        // global
        return this.getWord(state._global_vars + 2 * (variable - 16));
      }
    }
    ...
    getLenZString(addr) {
        let len = this.getByte(addr); addr ++;
        let chars = [];
        while (len-- > 0) {
            let w = this.getWord(addr);
            chars.push((w >> 10) & 0x1f,
...
log.debug("     taking branch!");
    ...
    lookupToken(dict, encoded_token_words) {
        ...
        let entry_len = this.getByte(dict); dict ++;

        let num_entries = this.getWord(dict); dict += 2;
        if (num_entries < 0) {
    ...
    encodeToken(text, padding=0x05) {
        ...
        for (let i = 0; i < text.length; i ++) {
            if (text[i] < 'a' || text[i] > 'z') throw new Error("encodeToken is too dumb");
    ...
    dumpParsebuffer(parsebuffer) {
        let max = this.getByte(parsebuffer); parsebuffer++;
        let count = this.getByte(parsebuffer); parsebuffer++;
fs.writeSync(log_fp, ` max = ${max}, count = ${count} tokens = [`);
        for (let i = 0; i < count; i ++) {
            let addr = this.getWord(parsebuffer); parsebuffer+=2;
            let length = this.getByte(parsebuffer); parsebuffer++;
            let from = this.getByte(parsebuffer); parsebuffer++;
